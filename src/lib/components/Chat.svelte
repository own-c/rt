<script>
    ...
    import { stream, initChat } from "$lib/Stream.svelte";
    import { regex, emotes } from "$lib/Emotes.svelte";
    ...
    function runRegex(text) {
        ...
        if (!regex[stream.username]) return text;

        return text.replace(regex[stream.username], (match) => {
            const emote = emotes[stream.username][match];
            return emote
...
<div class="relative" style="height: calc(100vh - 2.0rem);">
    ...
    >
        {#each messages as message}
            ...
            >
                <span class="font-bold {getMemberColor(message.u)}"
                    >{message.u}</span
                ><!--
      --><span class="text-white">:&nbsp;</span
                ><!--
      --><span class="text-white"
                    >{@html runRegex(message.m)}</span
                >
            </div>
        {/each}
    </div>
